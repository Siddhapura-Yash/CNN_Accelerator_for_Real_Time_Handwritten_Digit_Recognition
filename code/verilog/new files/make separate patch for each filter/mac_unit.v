...
module mac #(parameter DATA_WIDTH = 8, KERNEL_SIZE = 3)(
    input clk,
    input rst,
    input signed [DATA_WIDTH*KERNEL_SIZE*KERNEL_SIZE-1:0]data_in,
...
wire [2*DATA_WIDTH-1:0]temp[0:KERNEL_SIZE*KERNEL_SIZE-1];
...
generate
    ...
    begin
        assign temp[i] = $signed(data_in[i*DATA_WIDTH +: DATA_WIDTH]) * $signed(weight[i*DATA_WIDTH +: DATA_WIDTH]);
    end
endgenerate
